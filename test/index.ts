...
describe('throttle', () => {
  ...
  const handleUnhandledRejection = (err: any) => unhandledRejections.push(err)
  before(() => {
  ...
  it('works with sync function', async function (): Promise<void> {
    const foo = sinon.spy((a: number): number => {
    ...
    for (const promise of promises) expect(promise.value()).to.equal(6)
    ...
    for (const promise of promises) expect(promise.value()).to.equal(10)
  ...
  it('works', async function (): Promise<void> {
    ...
    promises = [fn(1, 200)].map(wrapPromise)
    await clock.tickAsync(1)
    ...
    for (const i of [1, 2]) assert(promises[i].isPending())
    await clock.tickAsync(200)
    for (const i of [1, 2]) expect(promises[i].isRejected()).to.be.true
  })
  it('supports getNextArgs option', async function (): Promise<void> {
    ...
    const fn = throttle(foo, 100, {
      getNextArgs: ([a], [b]) => [Math.min(a, b)] as const,
    })
    ...
    await clock.tickAsync(1)
    for (const promise of promises) expect(promise.value()).to.equal(2)
    ...
    await clock.tickAsync(60)
    for (const promise of promises) expect(promise.isRejected()).to.be.true
  ...
  it('.cancel', async function () {
    ...
    const fn = throttle(foo, 100)
    const promises: WrappedPromise<number>[] = []
